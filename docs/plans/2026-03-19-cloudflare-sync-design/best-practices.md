...
```bash
# 步骤 1：登录 Cloudflare（一次性，OAuth 浏览器授权）
wrangler login
...
`event sync init` 自动完成：
1. 调用 Cloudflare API 创建 D1 数据库（`event-sync-db`）
2. 初始化数据库 Schema
3. 上传并部署 Worker 代码（CLI 内置）
4. 生成随机 Bearer Token，通过 Cloudflare API 写入 Worker secret
5. 生成 AES-256 加密主密钥，存入 macOS Keychain
6. 保存 Worker URL 到 `~/.config/event/config.toml`

完成后输出 Linux 环境变量（用于 Agent 配置）：
```
✅ Cloudflare D1 已创建：event-sync-db
✅ Worker 已部署：https://event-sync-abc123.workers.dev
✅ 加密密钥已生成并存入 Keychain

Linux/Agent 配置（在目标机器上执行）：
  export CLOUDFLARE_API_URL="https://event-sync-abc123.workers.dev"
  export CLOUDFLARE_API_TOKEN="<auto-generated-token>"
  export EVENT_ENCRYPTION_KEY="<base64-key>"
```

### event sync init 内部实现流程

```swift
actor SyncInitService {
    func initialize() async throws {
        // 1. 验证 wrangler 已登录
        try await verifyWranglerAuth()

        // 2. 创建 D1 数据库（Cloudflare REST API）
        let dbId = try await createD1Database(name: "event-sync-db")

        // 3. 初始化 Schema
        try await executeSQLSchema(dbId: dbId)

        // 4. 生成随机 Bearer Token
        let bearerToken = generateSecureToken()  // 32-byte random, base64

        // 5. 部署 Worker（内置 Worker 代码作为 Swift 资源）
        let workerURL = try await deployWorker(dbId: dbId, token: bearerToken)

        // 6. 生成加密密钥，存入 Keychain
        let encryptionKey = SymmetricKey(size: .bits256)
        try storeInKeychain(encryptionKey)

        // 7. 保存配置
        try saveConfig(CloudflareConfig(apiURL: workerURL, keySource: .keychain))

        // 8. 输出 Linux 环境变量
        printLinuxSetupInstructions(
            url: workerURL,
            token: bearerToken,
...
- `wrangler login` 使用 OAuth，凭据存储在 `~/.wrangler/config/`，CLI 通过读取该配置调用 Cloudflare API
- Bearer Token 由 CLI 随机生成，用户永远不需要手动设置
- 用户需要保存 Linux 输出的环境变量，特别是 `EVENT_ENCRYPTION_KEY`（丢失后无法解密历史数据）
- 重新运行 `event sync init` 会重新部署 Worker，但会复用已有的 D1 数据库和加密密钥
